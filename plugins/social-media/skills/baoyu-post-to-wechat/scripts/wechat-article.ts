...
async function copyImageToClipboard(imagePath: string): Promise<void> {
  ...
  const result = spawnSync('/home/kewang/.bun/bin/bun', [copyScript, 'image', imagePath], { stdio: 'inherit' });
...
async function parseMarkdownWithPlaceholders(
  ...
  const result = spawnSync('/home/kewang/.bun/bin/bun', args, { stdio: ['inherit', 'pipe', 'pipe'] });
  if (result.status !== 0) {
...
export async function postArticle(options: ArticleOptions): Promise<void> {
  ...
  try {
    ...
    if (!chrome) {
      ...
      if (wechatTab) {
        ...
        if (currentUrl.includes('appmsg_edit')) {
          console.log('[wechat] Existing tab is already an editor tab, reusing it');
        } else if (!currentUrl.includes('/cgi-bin/home')) {
          console.log('[wechat] Navigating to home...');
          await evaluate(session, `window.location.href = 'https://mp.weixin.qq.com/cgi-bin/home?t=home/index'`);
          await sleep(5000);
        } else {
          console.log('[wechat] Already at home page');
        }
      } else {
    ...
    // Menu element may have changed. Try to find it, but if not found, navigate directly.
    console.log('[wechat] Checking for home menu...');
    const menuExists = await evaluate(session, '!!document.querySelector(".new-creation__menu")');
    
    if (!menuExists) {
      // Menu not found - try alternative selectors or navigate directly
      console.log('[wechat] Home menu not found, trying alternative selectors...');
      const altMenu = await evaluate(session, `
        (() => {
          const selectors = ['[class*="create"]', '[class*="publish"]', '[class*="editor"]', '.nav_item', '[class*="menu"]'];
          for (const sel of selectors) {
            const els = document.querySelectorAll(sel);
            if (els.length > 0) return {selector: sel, count: els.length, first: els[0].textContent?.trim().substring(0,50)};
          }
          return null;
        })()
      `);
      console.log(`[wechat] Alt menu: ${altMenu}`);
    }

    // Look for an already-open editor tab first
    const allTargets = await cdp.send<{ targetInfos: Array<{ targetId: string; url: string; type: string }> }>('Target.getTargets');
    const editorTab = allTargets.targetInfos.find(
      t => t.url.includes('appmsg_edit') && !t.url.includes('action=list') && t.url.includes('type=77')
    );

    if (editorTab) {
      // Attach to existing editor tab
      console.log('[wechat] Found existing editor tab, attaching...');
      const { sessionId: editorSid } = await cdp.send<{ sessionId: string }>('Target.attachToTarget', {
        targetId: editorTab.targetId,
        flatten: true,
      });
      session = { cdp, sessionId: editorSid, targetId: editorTab.targetId };
      ...
      const editorLoaded = await waitForElement(session, '#title', 45_000);
      if (!editorLoaded) throw new Error('Existing editor tab not ready (#title not found)');
    } else {
      // Navigate to editor using CDP Page.navigate (not JS window.location)
      console.log('[wechat] Navigating to editor via Page.navigate...');
      ...
      const editorLoaded = await waitForElement(session, '#title', 45_000);
      if (!editorLoaded) {
        // Last resort: try with window.location in the same tab
        console.log('[wechat] Page.navigate failed, trying window.location...');
        await evaluate(session, `window.location.href = 'https://mp.weixin.qq.com/cgi-bin/appmsg?t=media/appmsg_edit&action=edit&type=77&lang=zh_CN'`);

        // Wait and check
        await sleep(12000);
        const checkLoaded = await waitForElement(session, '#title', 30000);
        if (!checkLoaded) throw new Error('Editor did not load via any method');
      }
    }

    await sleep(3000);
    await waitForElement(session, '.ProseMirror', 20_000);
