...
```javascript
// ⚠️ Token may display as *** or empty in tab.url, but the actual value is present and works.
// Always extract from URL — localStorage does NOT store WeChat session tokens.
const token = tab.url.match(/token=([^&]+)/)?.[1];
// Use it directly in XHR calls — it succeeds even when displayed as ***

// ❌ WRONG — localStorage extraction returns NOT FOUND for WeChat session tokens
...
// Strip cover images (data URI or any img with alt=封面图)
// ⚠️ Only strip cover/data-URI images, NOT body content images
body = body.replace(/<img[^>]*src\s*=\s*["']data:image\/[^"']*["'][^>]*>/gi, '');
body = body.replace(/<img[^>]*alt\s*=\s*["']封面图["'][^>]*>/gi, '');
// Strip h1 (title goes in title0 field, not content0)
body = body.replace(/<h1[^>]*>[\s\S]*?<\/h1>/gi, '');
// Strip copy hint divs
body = body.replace(/<div style="font-size:13px[^>]*>[\s\S]*?<\/div>/gi, '');
body = body.replace(/<hr[^>]*>/gi, '');
body = body.trim();
```

**⚠️ Do NOT strip `<svg>` elements** — inline SVGs are body content (architecture diagrams, flowcharts, illustrations). They are preserved by `filter_content_html` and render correctly in WeChat. Only strip cover images (data URI `<img>` tags).
